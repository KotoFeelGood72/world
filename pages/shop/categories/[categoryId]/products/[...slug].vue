					...
					<div class="single_content">
						...
						<div class="single_action">
							<Button
								...
								:active="isInCart"
							/>
							<div class="single_like" @click="toggleLike" :class="{ active: isLiked }">
								<Icon :name="isLiked ? 'ph:heart-fill' : 'ph:heart-light'" size="60" />
							</div>
						</div>
						<div class="single_desc">
							<div class="single_desc__row">
								<span>Состав:</span>
								<p>{{ product.contents.short_description }}</p>
							</div>
							<div class="single_desc__row">
								<span>Описание:</span>
								<p>{{ product.contents.txt }}</p>
							</div>
						</div>
					</div>
				</div>
...
<script setup lang="ts">
	...
	const toggleCart = () => {
		const selectedAttributes = product.value.attributes.map((attr: any) => {
			const selectedOption =
				attr.slug === "pa_color" ? colorModel.value : sizeModel.value;
			const selectedValue = attr.values.find(
				(value: any) => value.slug === selectedOption
			);

			return {
				name: attr.name,
				option: selectedOption,
				id: selectedValue ? selectedValue.id : null,
			};
		});
		...
		const selectedProduct = {
			...
			image: (product.value as any).images[0],
	...
	async function getProduct() {
		...
		try {
			const response = await $domain.get(
				`/wp-json/single/v1/product/${route.params.slug}`
			);
			...
			if (product.value.attributes) {
				const sizeAttr = product.value.attributes.find((attr: any) =>
					attr.name.toLowerCase().includes("размер")
				);
				const colorAttr = product.value.attributes.find((attr: any) =>
					attr.name.toLowerCase().includes("цвет")
				);

				if (sizeAttr) {
					sizes.value = sizeAttr.values.map((value: any) => ({
						label: value.name,
						value: value.slug,
						id: value.id,
					}));
					sizeModel.value = sizes.value[0].value;
				}

				if (colorAttr) {
					colors.value = colorAttr.values.map((value: any) => ({
						label: value.name,
						value: value.slug,
						id: value.id,
					}));
					colorModel.value = colors.value[0].value;
				}
			}
		} catch (error) {
