    ...
    <div class="container">
      <div class="header_main">
        ...
        <div class="header_actions">
          <ul>
            <li @click="toast.success('Good')">
              <Icon name="custom:search" size="24" />
            </li>
            <li>
              <nuxt-link to="/favorite">
                <Icon name="custom:heart" size="24" />
                <div class="counter" v-if="isLikesLength">{{ isLikesLength }}</div>
              </nuxt-link>
            </li>
            <li>
              <nuxt-link to="/cart">
                <Icon name="custom:bag" size="24" />
                <div class="counter" v-if="isCartsLength">{{ isCartsLength }}</div>
              </nuxt-link>
            </li>
          </ul>
        </div>
...
.header {
  ...
  &.fixed-header {
    @include bp($point_2){
      ...
      border-bottom: .1rem solid #8888883f
    }
  }
}
